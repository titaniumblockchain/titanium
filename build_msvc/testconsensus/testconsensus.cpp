...
// etitanium includes.
#include <..\src\script\etitaniumconsensus.h>
...
int main()
{
    std::cout << "etitaniumconsensus version: " << etitaniumconsensus_version() << std::endl;
    ...
    auto op0Result = etitaniumconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, etitaniumconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
