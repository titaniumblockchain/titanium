...
"""eTitanium test framework primitive and message structures

CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    etitanium/primitives
...
COIN = 100000000  # 1 etit in satoshis
...
# Objects that map to etitaniumd objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/etitanium/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in etitaniumd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
    ...
    def is_valid(self):
        ...
        for tout in self.vout:
            if tout.nValue < 0 or tout.nValue > 105000000 * COIN:
                return False
        return True
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in etitaniumd indicates these should be deserialized as blocks
