...
#include <qt/etitaniumaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a eTitanium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new eTitaniumAddressEntryValidator(parent));
    widget->setCheckValidator(new eTitaniumAddressCheckValidator(parent));
}

bool parseeTitaniumURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no etitanium: URI
    if(!uri.isValid() || uri.scheme() != QString("etitanium"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!eTitaniumUnits::parse(eTitaniumUnits::ETIT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseeTitaniumURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseeTitaniumURI(uriInstance, out);
}

QString formateTitaniumURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("etitanium:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(eTitaniumUnits::format(eTitaniumUnits::ETIT, info.amount, false, eTitaniumUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openeTitaniumConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ETITANIUM_CONF_FILENAME));
    ...
    /* Open etitanium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "eTitanium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "eTitanium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("eTitanium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for eTitanium*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "etitanium.desktop";
    return GetAutostartDir() / strprintf("etitanium-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a etitanium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=eTitanium\n";
        else
            optionFile << strprintf("Name=eTitanium (%s)\n", chain);
