...
###   etitaniumcore.org and etitanium.org and compares them.
...
WORKINGDIR="/tmp/etitanium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://etitaniumcore.org"
HOST2="https://etitanium.org"
BASEDIR="/bin/"
VERSIONPREFIX="etitanium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'etitanium-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "etitanium.org failed to provide signature file, but etitaniumcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "etitanium.org and etitaniumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the eTitanium Core binary release signing key installed?"
   fi
