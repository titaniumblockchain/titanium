...
# TestP2PConn: A peer we use to send messages to etitaniumd, and store responses.
...
class CompactBlocksTest(eTitaniumTestFramework):
    ...
    # This test actually causes etitaniumd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that etitaniumd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for etitaniumd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # etitaniumd will not send blocktxn responses for blocks whose height is
