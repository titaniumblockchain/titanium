...
const char * const ETITANIUM_CONF_FILENAME = "etitanium.conf";
const char * const ETITANIUM_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/etitanium/etitanium/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //etitanium-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ETITANIUM_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "etitanium";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\eTitanium
    // macOS: ~/Library/Application Support/eTitanium
    // Unix-like: ~/.etitanium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "eTitanium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/eTitanium";
#else
    // Unix-like
    return pathRet / ".etitanium";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ETITANIUM_CONF_FILENAME);
