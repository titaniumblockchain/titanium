...
The eTitanium Core daemon can be configured to act as a trusted "border
router", implementing the etitanium wire protocol and relay, making
...
The ZeroMQ feature in eTitanium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building etitaniumd:
...
For instance:
    ...
    $ etitaniumd -zmqpubhashtx=tcp://127.0.0.1:234707 \
               -zmqpubhashtx=tcp://192.168.1.2:234707 \
               -zmqpubrawtx=ipc:///tmp/etitaniumd.tx.raw \
...
These options can also be provided in etitanium.conf.
...
From the perspective of etitaniumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into etitaniumd directly. Furthermore, no information is
...
using. eTitaniumd appends an up-counting sequence number to each
